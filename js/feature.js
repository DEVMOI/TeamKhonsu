      ...
      success: function twitchData(data) {
        ...
        if(data.stream !== null){
          ...
          //IF EVERYTHING IS A SUCCESS IT BUILDS A STREAMCARD//
            if (streamLogo !== null){
              streamCard = "<div class='online overlay'>" + "<a target='myiframe' href='https://player.twitch.tv/?channel="+twitchUserName+"&autoplay=false'><div class='card cardList cardFeature'><span class='tag tag-success status-bar'>ONLINE</span>" + "<div class='card-block'>" + "<img class='round-logo img-responsive logo' src='" + streamLogo + "' alt='" + twitchUserName + "logo'>" + '<br>' + "<div class='text-padding'>" + "<p class='card-title'>" + twitchUserName.toUpperCase() + "</p><p class='card-text'><strong>" + streamGame.toUpperCase() + ": </strong>" + streamInfo.toUpperCase() + "</p></a></div></div></div></div>";
              $(".card-container").append(streamCard);
            }else{
              streamCard = "<div class='online overlay'>" + "<a target='myiframe' href='https://player.twitch.tv/?channel="+twitchUserName+"&autoplay=false'><div class='card cardList cardFeature'><span class='tag tag-success status-bar'>ONLINE</span>" + "<div class='card-block'>" + "<img class='round-logo img-responsive logo' src='" + defaultImage + "' alt='" + twitchUserName + "logo'>" + '<br>' + "<div class='text-padding'>" + "<p class='card-title'>" + twitchUserName.toUpperCase() + "</p><p class='card-text'><strong>" + streamGame.toUpperCase() + ": </strong>" + streamInfo.toUpperCase() + "</p></a></div></div></div></div>";
              $(".card-container").append(streamCard);
            }
                  ...
                  success: function(data){
                    ...
                    if(streamMessage == "Unprocessable Entity"){
                      ...
                      $(".card-container").append(streamCard);
                        ...
                        streamCard = "<div class=' offline overlay'>" + "<a target='myiframe' href='https://player.twitch.tv/?channel="+twitchUserName+"&autoplay=false'><div class='card cardList cardFeature'>" + "<img class='theCard cardListIMG img-fluid stream-cap' src='" + defaultImage + "' alt='" + twitchUserName + " image capture'><iframe class='chat' src='https://www.twitch.tv/" + twitchUserName + "/chat?popout=' frameborder='0' scrolling='no'></iframe>" + "<br><span class='tag tag-default status-bar'>OFFLINE</span>" + "<div class='card-block'>" + "<img class='round-logo img-responsive logo' src='" + defaultThumb + "' alt='" + twitchUserName + "logo'>"+ '<br>' + "<div class='text-padding'>" + "<p class='card-title'>" + twitchUserName.toUpperCase() + "</p><p class='card-text'>Not currently streaming.</p></div></div></div></a></div>";
                        $(".card-container").append(streamCard);
                    }
